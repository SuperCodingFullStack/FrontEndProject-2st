    ...
    <div className="전체영역 세로정렬">
      <div className="세부 조정이 필요한 영역 가로정렬">
        <input
          type="checkbox"
          checked={isSelected}
          onChange={() => onProductSelect(product.product_item_name)} // 제품 체크박스 상태 변경 처리
        />
        <div className="cart_content_img">{product.imageUrl}</div>
        <div className="cart_content_list">
          <div className="product_name">
            <div>{product.product_item_name}</div>
            <div style={{ color: "rgb(190,191,190)", cursor: "pointer" }}>
              ×
            </div>{" "}
            {/* 삭제 기능 구현 필요 */}
          </div>
          <span style={{ color: "#6d747b" }}>{product.product_price}원</span>
          <span style={{ fontWeight: "bold" }}>{product.quantity}개</span>
        </div>
      </div>
